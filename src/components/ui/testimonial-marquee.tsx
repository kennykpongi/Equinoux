...
/**
 * Vendored from an external component source. Two deviations, both noted so a
 * future update from upstream can be reconciled:
 *
 *   1. `role` is declared on the Testimonial interface but was never rendered.
 *      Both card variants now fall back to it when there is no `username` —
 *      an attribution without the person's position is worth much less.
 *   2. Each raw <img> carries an eslint-disable for @next/next/no-img-element.
 *      The avatars here are data URIs, which next/image cannot optimise
 *      anyway, so the raw tag is correct rather than merely convenient.
 */
